let crates = require('fs').readFileSync('input', 'utf-8').split(/\r?\n/),
...
for(let i = 0; i < nbPiles - crates.length; i++) crates.unshift([])  //make crates matrix square so we can rotate it
...
for(let step of steps){
    for (let i = 0; i< step[0]; i++){
        crates[step[2]].push(crates[step[1]].pop())
    }
}
